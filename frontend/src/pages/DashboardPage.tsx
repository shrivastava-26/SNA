...
function SectionHeader({ title, count }: { title: string; count?: number }) {
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
      ...
      {count !== undefined && (
        <Chip label={count} size="small" sx={{ bgcolor: '#e2e8f0', fontWeight: 700, fontSize: '0.72rem' }} />
      )}
    </Box>
  );
}
...
export function DashboardPage() {
  ...
  // Status doughnut
  const statusCounts = studies.reduce<Record<string, number>>((acc, s) => {
  ...
  const doughnutData = {
    ...
    datasets: [{
      ...
      backgroundColor: ['#0f766e', '#d97706', '#64748b', '#dc2626'],
      borderWidth: 2,
      borderColor: '#fff',
    }],
  };
  ...
  const doughnutOptions = {
    plugins: {
      legend: { position: 'bottom' as const, labels: { font: { family: 'Poppins', size: 12 }, padding: 12 } },
    },
    cutout: '65%',
  };

  // Phase bar
  const phaseCounts = studies.reduce<Record<string, number>>((acc, s) => {
  ...
  const barData = {
    ...
    datasets: [{
      ...
      backgroundColor: ['#0f766e', '#14b8a6', '#0369a1', '#7c3aed'],
      borderRadius: 6,
    }],
  };
  ...
  // Specialty breakdown bar
  const specialtyCounts = examiners.reduce<Record<string, number>>((acc, e) => {
  ...
  const specialtyOptions = {
    ...
    plugins: { legend: { display: false } },
    scales: {
      x: { beginAtZero: true, ticks: { stepSize: 1, font: { family: 'Poppins' } } },
      y: { ticks: { font: { family: 'Poppins', size: 11 } } },
    },
  };

  // Country breakdown for sites
  const countryCounts = sites.reduce<Record<string, number>>((acc, s) => {
    ...
    <Layout>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Clinical trial portfolio overview — studies, sites, and examiners at a glance.
        </Typography>
      </Box>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 3 }}>
        <Box sx={{ flex: '1 1 200px' }}>
          <StatCard label="Total Studies" value={studies.length} sub={`${activeStudies} active · ${plannedStudies} planned`} icon={<ScienceIcon />} color="#0f766e" />
        </Box>
        <Box sx={{ flex: '1 1 200px' }}>
          <StatCard label="Completed Studies" value={completedStudies} sub="Successfully concluded" icon={<AssignmentTurnedInIcon />} color="#16a34a" />
        </Box>
        <Box sx={{ flex: '1 1 200px' }}>
          <StatCard label="Active Sites" value={activeSites} sub={`of ${sites.length} total sites`} icon={<LocationOnIcon />} color="#0369a1" />
        </Box>
        <Box sx={{ flex: '1 1 200px' }}>
          <StatCard label="Active Examiners" value={activeExaminers} sub={`of ${examiners.length} total examiners`} icon={<PersonSearchIcon />} color="#7c3aed" />
        </Box>
      </Box>

      {/* ── Row 2: Status summary pills ── */}
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: 2, border: '1px solid #e2e8f0', display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
          ...
          <Typography variant="body2" color="text.secondary">{activeSites} active sites</Typography>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
          <PendingActionsIcon sx={{ fontSize: 16, color: '#d97706' }} />
          <Typography variant="body2" color="text.secondary">{sites.length - activeSites} inactive sites</Typography>
        </Box>
      </Paper>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 3 }}>
        ...
        <Box sx={{ flex: '1 1 220px', minWidth: 0 }}>
          <Paper elevation={0} sx={{ p: 3, borderRadius: 2, border: '1px solid #e2e8f0' }}>
            ...
            <Box sx={{ position: 'relative', height: 260 }}>
              <Bar data={specialtyBar} options={{ ...specialtyOptions, maintainAspectRatio: false }} />
            </Box>
          </Paper>
        </Box>
      </Box>

      {/* ── Row 4: Recent studies + Countries ── */}
          ...
          <Paper elevation={0} sx={{ p: 3, borderRadius: 2, border: '1px solid #e2e8f0' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {recentStudies.map((study) => (
                <Box
                  key={study.id}
                  sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2, py: 1.2, bgcolor: '#f8fafc', borderRadius: 1.5, border: '1px solid #e2e8f0' }}
                >
              ...
              {topCountries.map(([country, count]) => (
                <ListItem key={country} disablePadding sx={{ mb: 1 }}>
                  <ListItemAvatar sx={{ minWidth: 40 }}>
                    <Avatar sx={{ width: 32, height: 32, bgcolor: '#e0f2f1', color: '#0f766e' }}>
                      <PublicIcon sx={{ fontSize: 16 }} />
                    </Avatar>
                  </ListItemAvatar>
                  <ListItemText
                    primary={<Typography variant="body2" sx={{ fontWeight: 600 }} color="text.primary">{country}</Typography>}
                    secondary={<Typography variant="caption" color="text.secondary">{count} site{count > 1 ? 's' : ''}</Typography>}
                  />
                  <Box sx={{ minWidth: 28, height: 28, borderRadius: '50%', bgcolor: '#0f766e', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 700 }}>
                    {count}
                  </Box>
                </ListItem>
              ))}
